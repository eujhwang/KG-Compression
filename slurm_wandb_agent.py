...
def main(args):
    ...
    try:
        ...
        if not args.force:
            raise ValueError(f'Got sweep_id = {args.sweep_id}, but directory {job_dir} already exists!')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--conda_env', type=str, default="vns", help="conda env name to be activated")
    parser.add_argument('--num-jobs', type=int, default=1)
    parser.add_argument('--num-agents-per-job', type=int, default=1)
    parser.add_argument('--edit-sbatch', action='store_true', help="open the sbatch.sh file in a text editor before running")
    parser.add_argument('--edit-srun', action='store_true', help="open the srun.sh file in a text editor before running")
    parser.add_argument('--no-run', action='store_true', help="don't run the job(s), just create the sh files")
    parser.add_argument('-f', '--force', action='store_true', help="force creation of job folder if it already exists, potentially overwriting data")
    parser.add_argument('--slurm_dir', type=str, default="slurm", help="directory to store slurm files in")
    parser.add_argument('--srun-filename', type=str, default="srun.sh", help="filename of srun script (to be created)")
    parser.add_argument('--sbatch-filename', type=str, default="sbatch.sh", help="filename of sbatch script (to be created)")
    parser.add_argument('--srun-template', type=str, default="srun.sh", help="filename of srun template (which will be copied and appended to) relative to SLURM_DIR")
    parser.add_argument('--sbatch-template', type=str, default="sbatch.sh", help="filename of sbatch template (which will be copied and appended to) relative to SLURM_DIR")
